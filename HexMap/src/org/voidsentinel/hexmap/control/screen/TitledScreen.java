 ...
 * of the screen (by defaylt classname). Warning the id should be in lowercase,
 * even if defined with high and lowercase
 * <li>the font is defined by a "font" with "title.font"
 * <li>the font size is defined by a "data" with "title.fontsize"
 * <li>the font color is defined by a "data" with "title.textcolor"
 * <li>the title background defined by a "image" with "title.background"
 * </ul>
 * 
 * @see ImageRepository
 * @see FontRepository
...
public class TitledScreen extends GameState {
	...
	private void generateIHM() {
		...
		if (guiNode.getChild(CONTAINER_ID) == null) {
			...
			// Title
			titleContainer.setPreferredSize(new Vector3f(settings.getWidth(), 40f, 0f));
			...
			title.setPreferredSize(new Vector3f(settings.getWidth(), 40f, 0f));
