...
/**
 * A layout similar to the JavaFX HBox in which children are laid out horizontally
 ...
 * resizing the child elements horizontally when the HBoxLayout is bigger or smaller
 * than the sum of its children.
 * 
 * None - Children will not be resized horizontally, extra space is empty. This defaults
 *        to Proportional when children need to shrink.
 * Even - Remaining space is divided evenly among the children or the children are
 *        shrunk by equal amounts.
 * Proportional - Children are grown or shrunken by an amount proportional to their
 *        original size.
 * ForcedEven - The space in the HBoxLayout is divided evenly among all children.
 * First - The first child takes up all of the remaining space, if the children need
 *         to shrink the first child absorbs all of the shrinkage, if additional
 *         shrinkage is required the next child will shrink and so on.
 * Last - Same as First, except we start enlarging or shrinking from the bottom.
 * 
 * Child elements can be instructed to fill remaining vertical space or, by default,
 * remain their original vertical size. By default items that do not fill the
 * vertical size of the HBoxLayout will be aligned to the top, the default alignment
 * can be changed in the constructor and overridden by each added child. Additional
 * alignments are Bottom and Center.
 * 
...
public class HBoxLayout extends AbstractGuiComponent implements GuiLayout, Cloneable {
     ...
     * elements, <code>FillMode.None</code>, elements will not stretch to
    ...
    /**
     * Constructs a new <code>HBoxLayout</code> elements will not stretch to fill
     * remaining vertical space and are top aligned.
     * 
     * @param margin Amount of space between elements.
     * @param vFillMode <code>FillMode</code> indicating how horizontal stretching and
     ...
     * @param vFillMode <code>FillMode</code> indicating how horizontal stretching and
     * shrinking should work.
     * @param fillHeight <code>true</code> elements smaller than the height of the
     * <code>HBoxLayout</code> will be stretched to fill the height, otherwise
     * they remain their original size. This can be overridden by individual elements
     * when they are added.
     * 
     * @see #addChild(com.jme3.scene.Node, chet2.ui.components.HBoxLayout.VAlign, boolean) 
     */
     ...
     * @param fillHeight <code>true</code> elements smaller than the height of the
     * <code>HBoxLayout</code> will be stretched to fill the height, otherwise
     * they remain their original size. This value can be overridden by individual
     * elements when they are added.
     * @param defaultAlignment Elements are aligned horizontally either Left, Right
     * or Center. This can be overridden when adding an element.
     * 
     * @see #addChild(com.jme3.scene.Node, chet2.ui.components.HBoxLayout.VAlign, boolean)
     * @see VAlign
     */
    ...
    /**
     * Sets the method by which child elements will be grown or shrunk horizontally when
     * the <code>HBoxLayout</code> is bigger or smaller than the sum of element heights.
     * 
     * None - Children will not be resized horizontally, extra space is empty. This defaults
     *        to Proportional when children need to shrink.
     * Even - Remaining space is divided evenly among the children or the children are
     ...
     * @param fillHeight True if this element should fill remaining vertical space.
